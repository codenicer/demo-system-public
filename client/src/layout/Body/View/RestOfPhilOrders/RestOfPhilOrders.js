...
import { fetchRestOfPhil } from '../../../../scripts/actions/ordersActions'
import {
  handleLoadOrders,
  filterOrder,
  fitlerTextOnchange,
...
const RestOfPhilOrders = (props) => {
  ...
  function fetchData(mParams = null) {
    fetchRestOfPhil(mParams || params)
  }
  ...
  //check if params and filter changed
  useEffect(() => {
    setTableData(ordersRestOfPhil.rows)
    setOrderCount(ordersRestOfPhil.count)
    setSelectedID([])
  }, [ordersRestOfPhil])
      ...
      >
        ...
        >
          <div className="grd gtc-af grd-gp-3 jis">
            <span className="header asc">Rest of the Philippines</span>
          </div>
          <div
            style={{
        ...
        >
          <PendingOrderHeader
            bulk={bulk}
            data={
              ordersRestOfPhil !== null && ordersRestOfPhil.length > 0
                ? ordersRestOfPhil[0]
                : []
            }
            css={`
