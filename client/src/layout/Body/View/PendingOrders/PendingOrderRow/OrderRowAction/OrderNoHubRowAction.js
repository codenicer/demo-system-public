...
import {confirmPaid,holdOrder,cancelNoHubOrder,sentToFloristJob} from '../../../../../../scripts/actions/ordersActions'
import ModalCancel from '../../../../../../Pages/ModalCancel/ModalCancel';
import { faCheck, faHandPaper, faBan, faArrowRight } from '@fortawesome/free-solid-svg-icons'
...
const OrderNoHubRowAction = ({ css, rowData ,confirmPaid,holdOrder,cancelNoHubOrder,dispositions,sentToFloristJob}) => {
  ...
  const [holdForm ,setHold] = useState(holdFormInitialState)
    ...
    const handleCOSubmit = () =>{
        if(cancelForm['cancel_reason'].replace(/\s/g,'').length < 1){
            ...
            cancelNoHubOrder(cancelForm,rowData,
                    (type,text)=>{
                        handleCOClose()
                        toast[type](text)   
                }
        ...
        <>
            <div className='_action_row bg-white grd aic jic'>
                ...
                <IconButton
                    ...
                    />
                  {(parseInt(rowData.payment_status_id)  === 2 && rowData.order_status_id <=2  && !rowData.job_florist) && (rowData.payment_method !== 'CPU' || moment(rowData.delivery_date).format("YYYY-MM-DD") === moment(new Date()).format("YYYY-MM-DD")) &&
                    <IconButton
                      icon={faArrowRight}
                      size='18px'
                      shadow={true}
...
OrderNoHubRowAction.defaultProps = {
...
export default connect(transferStatetoProps,{confirmPaid,holdOrder,cancelNoHubOrder,sentToFloristJob})(OrderNoHubRowAction);