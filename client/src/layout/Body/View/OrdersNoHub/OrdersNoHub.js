...
import OrderNoHubHeader from '../PendingOrders/PendingOrderHeader/OrderNoHubHeader'
import OrderNoHubRow from '../PendingOrders/PendingOrderRow/OrderNoHubRow'
import { fetchOrdersNoHub } from '../../../../scripts/actions/ordersActions'
import {
  handleLoadOrders,
  filterOrder,
  fitlerTextOnchange,
...
const PendingOrders = (props) => {
  ...
  function fetchData(mParams = null) {
    fetchOrdersNoHub(mParams || params)
  }
  ...
  //check if params and filter changed
  useEffect(() => {
    setTableData(ordersNoHub.rows)
    setOrderCount(ordersNoHub.count)
    setSelectedID([])
  }, [ordersNoHub])
      ...
      >
        ...
        >
          <div className="grd gtc-af grd-gp-3 jis">
            <span className="header asc">Orders with No Hub</span>
          </div>
          <div
            style={{
        ...
        >
          <OrderNoHubHeader
            bulk={bulk}
            data={
              ordersNoHub !== null && ordersNoHub.length > 0
                ? ordersNoHub[0]
                : []
            }
            css={`
              ${bulk ? 'pending_template-bulk' : 'no_hub_template'} aic
            `}
          />
    ...
    orders.map((order, key) => {
      return (
        <OrderNoHubRow
          ...
          bulk={bulk}
          css={`
            ${bulk ? 'pending_template-bulk' : 'no_hub_template'}
          `}
          key={key}
          orders={orders}
          rowData={order}
