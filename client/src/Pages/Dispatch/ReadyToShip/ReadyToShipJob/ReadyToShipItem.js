...
import "./ReadyToShipItem.css";

const QueueJobItem = ({ data, failedIds, noteCounts, onRemove, css, qualityChecked, onQualityCheck, paymentHighlight }) => {
  let type = 1;
    ...
    <>
      <TableRow 
        ...
        <OrderID
          ...
          orderid={data["order_id"]}>{data.jobtype === 'delivery' ? data.shopify_order_name : `${data.shopify_order_name}-CPU`}</OrderID>
        ...
        <div
          className='pad-x-1' 
          style={{background: paymentHighlight === data['payment_method'] && 'var(--yellow)'}}>{data['payment_method']}</div>
