...
import ReadyToShipJob from "./ReadyToShipJob/ReadyToShipJob";
import Input from "../../../atoms/Input/Input";
import Button from "../../../atoms/Button/Button";
import {
  fetchReadyToShipJobs,
  deleteJob,
  deleteJobItem,
  shipDispatchJobVD,
  getTrackingNo,
...
//  let filterVal = props.location.search ? queryString.parse(props.location.search.replace('?', '')) : '';
  ...
  const {
    dispatchData: { ready_to_ship, tracking_data }
  } = props;
  ...
  function changeRiderDetails() {
    ...
    setRiderDetails(mparams, msg => {
      ...
      tableData[selectedIndex]['rider_id'] = riderForm.rider_id;
      tableData[selectedIndex]['rider_first_name'] = `${riderForm.first_name} `;
      tableData[selectedIndex]['rider_last_name'] = riderForm.last_name;
      tableData[selectedIndex]['rider_mobile_number'] = riderForm.mobile_number;
      tableData[selectedIndex]['rider_provider_id'] = riderForm.rider_provider_id;
      tableData[selectedIndex]['rider_provider_name'] = riderForm.rider_provider_name;
    });
  ...
  function showRiderModal(dispatch_job_id, tracking_no = "", key) {
    fetchAvailableRiders();
    setSelectedDispatchJob(dispatch_job_id);
    setSelectedIndex(key);
    const selectedDJ = _.filter(ready_to_ship.rows, { dispatch_job_id });
  ...
  useEffect(() => {
    // setTableData(for_assignment)
    // console.log("assigned jobs:", ready_to_ship);
    // console.log('dispatch data', props.dispatchData)
    // if(ready_to_ship.rows.length){
    setTableData(ready_to_ship.rows);
    //  }
    //setTableData(ready_to_ship.rows)
    ...
    if (ready_to_ship.rows.length) {
      ...
      if (ready_to_ship.rows.length && !riderList.length) {
        setRiderList(
          _.map(ready_to_ship.rows, rec => {
  ...
  function onShipJob(dispatch_data) {
    ...
    if (window.confirm("You are about to Ship this Job. Click OK to proceed")) {
      ...
      shipDispatchJobVD({ dispatch_data }, msg => {
        fetchData(ajParams);
          fetchReadyToShipJobs(ajParams);
        toast.success(msg);
      });
  ...
  return (
    ...
    >
      <div
        ...
        <span className='header asc'>Ship VD</span>
        <HubFilter
          getHubID={getHubID}
          maxBadgeCount={8}
        />
    ...
    { addItemModal &&
        <DispatchAddItem
          unMount={() => fetchReadyToShipJobs(ajParams)}
